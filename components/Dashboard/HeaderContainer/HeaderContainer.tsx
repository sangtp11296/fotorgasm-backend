...
const HeaderContainer: React.FC<Props> = ({user}) => {
    ...
    const handleAvatarUpdate = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            if(image){
                const res = await fetch('https://4esg1vvhi3.execute-api.ap-southeast-1.amazonaws.com/dev/get-presigned-url', {
                    method: 'POST',
                    body: JSON.stringify({
                        userID: user.id,
    ...
    // Handle Info Update
    const handleInfoUpdate = async (e: React.FormEvent<HTMLFormElement>,name: string, title: string) => {
        e.preventDefault();
        try {
            if(name && title){
                const res = await fetch(`https://4esg1vvhi3.execute-api.ap-southeast-1.amazonaws.com/dev/admin/${user.id}`, {
                    method: 'PUT',
                    body: JSON.stringify({
                        userID: user.id,
                        username: name,
  ...
  return (
    <div className={`${styles.headerContainer} ${styles.gridBlock}`}>
        <div className={styles.firstSection}>
            ...
            <form id='adminUpdate' className={styles.editForm} onSubmit={(e) => handleInfoUpdate(e, newName, newTitle)}>
                <label htmlFor='avatarUpload' 
                    ...
                    <input hidden id='avatarUpload' type="file" accept="image/*" onChange={handleAvatarUpdate}/>
                    <svg height={18} width={18} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <circle cx="12" cy="6" r="4" stroke="#ffffff" strokeWidth="1.5"></circle> <circle cx="17" cy="18" r="4" stroke="#ffffff" strokeWidth="1.5"></circle> <path d="M17 16.6667V19.3333" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M15.6665 18L18.3332 18" stroke="#ffffff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path opacity="0.5" d="M14 20.8344C13.3663 20.9421 12.695 21 12 21C8.13401 21 5 19.2091 5 17C5 14.7909 8.13401 13 12 13C13.7135 13 15.2832 13.3518 16.5 13.9359" stroke="#ffffff" strokeWidth="1.5"></path> </g></svg>
                </label>
                ...
                <label htmlFor='adminUpdateTitle' style={{
                    ...
                    <input placeholder={user.role} type={popupAdmin === 'title' ? 'text' : 'hidden'} id='adminUpdateTitle' onChange={(e)=>setNewTitle(e.target.value)} />
                </label>
                <label htmlFor='adminUpdatePassword' style={{
                    ...
                    <input placeholder='Password' type={popupAdmin === 'password' ? 'text' : 'hidden'} id='adminUpdatePassword' onChange={(e)=>setNewPass(e.target.value)} />
