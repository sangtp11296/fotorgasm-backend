  ...
  return (
    <div className={`${styles.interContainer} ${styles.gridBlock}`} ref={containerRef}>
      {editorMode ?
        <Slider {...settings}>
          {
            (!draft.coverUrl && !draft.coverThumbnail && !draftAlbum.coverUrl && !draftAlbum.coverThumbnail) ? 
            ...
            :
            <>
              {(draft.format === 'blog' || draft.format === 'video') && <PostCoverThumbnail cover={cover}/>}
              {(draftAlbum.format === 'album') && <AlbumCoverThumbnail cover={cover}/>}
            </>
          }
