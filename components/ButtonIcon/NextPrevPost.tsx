...
export const NextPrevPost = ({ next, prev }: { next: string, prev: string }) => {
  ...
  return (
    ...
    {
      next !== '' ?
      <button onClick={() => handleClick(next)}  className={`${styles.button} ${styles.nextPrev} ${styles.next}`}>
    ...
    {
      prev !== '' ?
      <button onClick={() => handleClick(prev)} className={`${styles.button} ${styles.nextPrev} ${styles.prev}`}>
